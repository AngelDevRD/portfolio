# Juegos (Flutter)

Todos distribuidos fuera de Google Play, mismo mecanismo que las apps (ver
[UPDATE_SYSTEM.md](./UPDATE_SYSTEM.md)).

## Snake Evolution

- **Slug:** `snake-evolution` · **Package ID:** `com.portfolio.snakeevolution.snake_evolution`
- **Repo:** [AngelDevRD/snake_evolution](https://github.com/AngelDevRD/snake_evolution) (privado)
- **Categoría:** Arcade
- **Última versión publicada:** v1.0.1 (2026-07-11)
- **Icono:** `public/apps/snake-evolution/icon.png`
- **Descripción:** versión modernizada del clásico snake, con Material 3 y persistencia local.

## Number Merge

- **Slug:** `number-merge` · **Package ID:** `com.portfolio.numbermerge.number_merge`
- **Repo:** [AngelDevRD/number_merge](https://github.com/AngelDevRD/number_merge) (privado)
- **Categoría:** Puzzle
- **Última versión publicada:** v1.0.1 (2026-07-11)
- **Icono:** `public/apps/number-merge/icon.png`
- **Descripción:** puzzle numérico inspirado en 2048.

## Memory Cards

- **Slug:** `memory-cards` · **Package ID:** `com.portfolio.memorycards.memory_cards`
- **Repo:** [AngelDevRD/memory_cards](https://github.com/AngelDevRD/memory_cards) (privado)
- **Categoría:** Puzzle
- **Última versión publicada:** v1.0.1 (2026-07-11)
- **Icono:** `public/apps/memory-cards/icon.png`
- **Descripción:** juego de memoria clásico (encontrar parejas).

## Tower

- **Slug:** `stack-tower` · **Package ID:** `com.portfolio.stacktower.stack_tower`
- **Repo:** [AngelDevRD/stack_tower](https://github.com/AngelDevRD/stack_tower) (privado)
- **Categoría:** Arcade
- **Última versión publicada:** v1.0.1 (2026-07-11)
- **Icono:** `public/apps/stack-tower/icon.png`
- **Descripción:** apilar bloques en movimiento; cada acierto suma altura y precisión.
- **Nota:** agregado al catálogo el 2026-07-11 (antes no existía en el portafolio pese a
  tener repo y release publicados). Es el ejemplo de referencia de "cómo agregar un
  proyecto nuevo" en [PROJECTS.md](./PROJECTS.md). El launcher de Android mostraba
  "stack_tower" (nombre interno) y el icono genérico de Flutter — corregido a "Tower" con
  el icono oficial en `v1.0.1`.
